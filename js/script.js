...
const submitForm = (event) => {
    ...
    if (formTaskId.value == null || formTaskId.value == '') {
        ...
        //new Taskid
        const task = new Task(nextId(), formTaskName.value, formDescription.value, formDueDate.value, editCard, removeCard, dragstartEvent, formstage.value)
        taskList.push(task);
        const element = task.generateHtmlCard();
        const parentElement = document.querySelector(`[data-stage-id="${task.stage}"]`);
        ...
        taskList.map((task, index) => {
            if (task.id == formTaskId.value) {
                ...
                if (task.stage == formstage.value) {
                    ...
                    const element = task.generateHtmlCard();
                    const parentElement = document.querySelector(`[data-stage-id="${formstage.value}"]`);
                    parentElement.appendChild(element);
